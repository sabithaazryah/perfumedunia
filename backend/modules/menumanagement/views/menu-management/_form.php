...
<div class="menu-management-form form-inline">
    ...
    <div class='col-md-4 col-sm-6 col-xs-12 left_padd type-sub'>
        <?= $form->field($model, 'main_menu_arabic')->dropDownList(ArrayHelper::map(MainCategory::find()->all(), 'id', 'main_category'), ['prompt' => 'select Main', 'class' => 'form-control']) ?>
...
//            var host = 'http://' + window.location.host + '/' + 'perfumedunia/product/product?submenu=';for local
            var host = 'http://' + window.location.host + '/' + 'product/product?submenu=';//for server
