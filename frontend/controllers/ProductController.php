...
class ProductController extends \yii\web\Controller {
    ...
    public function actionProduct($submenu) {
        $category = "";
        $searchModel = new ProductSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $dataProvider->pagination->pageSize = 42;
        ...
        if (!empty($submenu)) {
            ...
            $dataProvider->query->andWhere(['category' => $category->id]);
        }



        $categories = Category::find()->where(['status' => 1, 'main_category' => $category->main_category])->all();
